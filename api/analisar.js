...
- Responda SOMENTE com o JSON, sem markdown`;
  ...
  try {
    ...
    if (!raw) {
      console.error("Gemini resposta vazia:", JSON.stringify(geminiData));
      return res.status(502).json({ error: "Gemini retornou resposta vazia", detail: JSON.stringify(geminiData) });
    }

    const parsed = JSON.parse(raw);
    return res.status(200).json(parsed);
  } catch (e) {
    console.error("Erro ao processar resposta da IA:", e.message);
